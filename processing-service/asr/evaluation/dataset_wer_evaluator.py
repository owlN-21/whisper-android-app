...
from asr.evaluation.wer_calculator import WerCalculator
...
class DatasetWerEvaluator:
    ...
    def evaluate_records(
        ...
        for record in records:
            ...
            wer = self.wer_calculator.calculate(
            ...
            evaluated_records += 1
            print(f"evaluated: {record.sample_id}, wer={wer:.4f}")

        average_wer = self._calculate_average_wer(results)

        summary = DatasetWerSummary(
    ...
    def _calculate_average_wer(self, results: List[SampleWerResult]) -> float:
        if not results:
            return 0.0

        total_wer = sum(result.wer for result in results)
        return total_wer / len(results)

    def _write_detailed_report(self, results: List[SampleWerResult]) -> None:
    ...
    def _write_summary(self, summary: DatasetWerSummary) -> None:
        ...
        lines = [
            ...
            f"average_wer: {summary.average_wer:.6f}",
            f"average_wer_percent: {summary.average_wer * 100:.2f}%",
        ]
